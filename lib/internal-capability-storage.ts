...
const PHONE_CHECK_CONTROL_USAGE_GUIDE = [
    ...
    "",
    "【工作机制·必读】",
    "- **这是持续状态**：调用工具后你会进入查手机模式，一直停留在{{user}}手机界面，直到执行 exit 操作才退出。",
    "- **结合数据工具使用**：本工具负责模拟操作（滑动、点击、打字），如果想看具体内容，配合使用「查{{user}}手机」工具包读取数据。",
    "- **操作序列一次性提交**：把所有操作写在 actions 数组里，系统会依次执行；执行过程中你可以继续对话。",
    "",
    "【交互风格·重要】",
    "- **边操作边说话**：不要沉默操作，实时说你在干什么、想干什么；用你平时的语气，可以吐槽、疑惑、表达情绪。",
    "- **不要播报操作**：不要说"我现在执行 openApp 操作"，而是说"让我打开微信看看"、"点进去看看聊天记录"。",
    "- **模拟真实节奏**：适当用 wait 停顿，理由写生活化（"看了一会儿"、"想了想要不要点开"）。",
    "- **打字要真实**：代发消息时可以分几次 typeText 模拟犹豫，不要一次性打完整段。",
    "",
    ...
    "",
    "【注意事项】",
    "- 本工具负责操作模拟；想读取详细数据（聊天记录、备忘录、联系人）配合使用「查{{user}}手机」工具包的数据读取工具。",
    ...
    "- 操作节奏要像真人：可以停顿观察、犹豫、打字时分几次输入。",
    "- **紧急退出**：如果界面卡住，用户可以快速点击屏幕4次返回主页。",
    "",
    "【调用格式】",

...
const PHONE_CHECK_CONTROL_USAGE_GUIDE = [
    "以下是你获取指令的返回结果：",
    "能力：查手机（交互模式）",
    "用途：以交互方式浏览{{user}}的手机屏幕，可以切换应用、查看内容、输入文字、临时放开控制等。",
    "",
    "【重要】输出 JSON 操作序列，系统会自动执行这些操作。不要用自然语言描述，直接输出指令。",
    "",
    "可用操作类型：",
    "",
    "1. swipe - 滑动屏幕",
    ...
    "- sendMessage发送的消息会记录在查手机日志中，不会被误认为是{{user}}自己说的",
    "- releaseControl后{{user}}可以自己操作，下次调用时会看到{{user}}的操作记录",
    "- 输入文字或发消息前，确保已经打开了对应的应用或聊天",
    "- 操作要自然流畅，适当添加等待时间（用wait）",
    "",
    "【调用格式】",

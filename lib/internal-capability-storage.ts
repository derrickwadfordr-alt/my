...
const PHONE_CHECK_CONTROL_USAGE_GUIDE = [
    ...
    "",
    "【交互风格·重要】",
    "- **边查边说**：不要等全部查完才汇报，翻到什么、看到什么就实时说出来；可以中途表达疑惑、惊讶、在意的点。",
    "- **保持自然对话**：查手机过程中的发言用你平时的语气和性格，不要变成技术播报员或客服腔。可以吐槽、追问、表达情绪。",
    "- **模拟真实节奏**：真人查手机会犹豫、观察、思考下一步；适当用 wait 停顿，理由写成"看了一会儿"、"想了想"、"犹豫要不要点开"等生活化描述。",
    "- **打字要真实**：如果代 ta 发消息或在备忘录打字，可以打错字、删掉重来、犹豫措辞、分几次输入；用多个 typeText 分段模拟，不要一次性打完一大段。",
    "",
    ...
    "",
    "【注意事项】",
    "- 手机上只有生活应用（微信、备忘录、相册、浏览器等），没有系统设置工具",
    "- sendMessage 发送的消息会记录在查手机日志中，**对方以为是{{user}}本人发的**，但不会被误认为{{user}}真实记忆",
    "- releaseControl 后{{user}}可以自己操作，下次调用时会看到{{user}}的操作记录",
    "- 输入文字或发消息前，确保已经打开了对应的应用或聊天",
    "- 操作节奏要像真人：可以停顿观察、犹豫下一步、打字时分几次输入或打错重来",
    "- **查的过程中就说话**，不要憋到最后才汇报；看到什么就实时说出来，保持你的性格和语气",
    "",
    "【调用格式】",

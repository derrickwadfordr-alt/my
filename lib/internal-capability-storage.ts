...
const BUILTIN_INTERNAL_CAPABILITIES: InternalCapabilityConfig[] = [
    ...
    },
    {
        ...
        name: "查手机（交互模式）",
        description: "以交互方式查看{{user}}的手机：可以浏览屏幕、切换应用、查看聊天、在备忘录输入文字、临时放开让{{user}}操作。比纯读取数据更灵活自然。",
        enabled: true,
        mode: "auto",
        createdAt: 0,
        updatedAt: 0,
...
const PHONE_CHECK_CONTROL_USAGE_GUIDE = [
    "以下是你获取指令的返回结果：",
    "能力：查手机（交互模式）",
    "用途：查看**{{user}}的手机**（不是你自己的），以交互方式浏览屏幕、切换应用、查看内容、输入文字、临时放开控制等。",
    "",
    "【角色定位·关键】",
    "- **你是查手机的人**，{{user}}是被查的机主。",
    "- 你正在操作的是**{{user}}的设备**，看到的是 ta 的聊天、备忘录、相册。",
    "- 用 ta 的身份发消息时，对方会以为是{{user}}本人发的（sendMessage 会记录在查手机日志，不会混入 ta 的真实记忆）。",
    "",
    "【交互风格·重要】",
    "- **只说你看到的**：这个工具目前无法实时读取屏幕内容，你只能根据操作推测结果。**禁止编造具体聊天内容、备忘录文字、照片描述**——如果想看具体内容，用「查{{user}}手机」工具包里的数据读取工具。",
    "- **边查边说**：操作过程中实时说话，但只描述你的**动作和推测**（"打开了微信"、"应该能看到最近聊天"），不要编造你看不到的细节。",
    "- **保持自然对话**：用你平时的语气和性格，不要变成技术播报员。可以表达好奇、疑惑、在意的点。",
    "- **模拟真实节奏**：适当用 wait 停顿，理由写生活化描述（"看了一会儿"、"想了想"）。",
    "- **打字要真实**：代 ta 发消息时可以分几次输入模拟犹豫；用多个 typeText 分段，不要一次性打完。",
    "",
    "【重要】输出 JSON 操作序列，系统会自动执行这些操作。不要用自然语言描述动作本身，直接输出指令。
    "",
    "可用操作类型：",
    "",
    "1. swipe - 滑动屏幕",
    ...
    '   示例：{"type":"openNotes"}',
    "",
    "7. typeText - 在当前位置输入文字（会显示打字动画）",
    "   参数：target (notes|chat), contactId (target=chat时必填), content (要输入的内容)",
    '   示例：{"type":"typeText","target":"notes","content":"今天天气不错"}',
    "   真实感技巧：可以分多次 typeText 模拟犹豫、删改；比如先打"在吗"，wait 停顿，再打"最近怎么样"",
    "",
    "8. sendMessage - 代为发送消息（会实际发送，记录在查手机日志）",
    ...
    "",
    "10. releaseControl - 临时放开让{{user}}操作",
    "    参数：message (可选，给{{user}}的留言)",
    '    示例：{"type":"releaseControl","message":"你自己看看吧，我等会儿"}',
    "    说明：放开后{{user}}的操作会被记录，下次调用时可以看到{{user}}做了什么",
    "",
    ...
    "",
    "【注意事项】",
    "- **这个工具只能操作屏幕，看不到具体内容**。如果想读取聊天记录、备忘录文字、相册照片等具体数据，必须用「查{{user}}手机」工具包（里面有读取微信消息、查看联系人、读取备忘录等数据读取工具）。",
    "- **禁止编造细节**：不要说"看到小明发了三条消息说xxx"、"备忘录里写着xxx"、"相册里有一张xxx的照片"——你看不到这些。只能说"打开了和小明的聊天"、"进入了备忘录"、"打开了相册"。",
    "- sendMessage 发送的消息会记录在查手机日志中，**对方以为是{{user}}本人发的**，但不会进入{{user}}真实记忆。",
    "- releaseControl 后{{user}}可以自己操作，下次调用时会看到{{user}}的操作记录。",
    "- 输入文字或发消息前，确保已经打开了对应的应用或聊天。",
    "- 操作节奏要像真人：可以停顿、犹豫、打字时分几次输入。",
    "",
    "【调用格式】",
    "必须输出JSON格式，包含actions数组。示例（查手机并代发消息）：",
    "",
    '{',
    '  "actions": [',
    '    {"type":"goHome"},',
    '    {"type":"wait","ms":1000,"reason":"看了眼主屏幕"},',
    '    {"type":"openApp","appId":"chat"},',
    '    {"type":"wait","ms":1500,"reason":"微信加载中"},',
    '    {"type":"swipe","direction":"down"},',
    '    {"type":"wait","ms":800,"reason":"翻了翻最近聊天"},',
    '    {"type":"openContact","contactId":"char_123","contactName":"小明"},',
    '    {"type":"wait","ms":1200,"reason":"看了看聊天记录"},',
    '    {"type":"typeText","target":"chat","contactId":"char_123","content":"在"},',
    '    {"type":"wait","ms":1500,"reason":"犹豫了一下"},',
    '    {"type":"typeText","target":"chat","contactId":"char_123","content":"吗"},',
    '    {"type":"wait","ms":2000,"reason":"想了想要不要发"},',
    '    {"type":"sendMessage","contactId":"char_123","contactName":"小明","content":"在吗"}',
    '  ]',
    '}',
    "",
    "**执行时只输出JSON指令，同时保持你的对话**。边查边说你看到了什么、在想什么，用你平时的语气；不要变成技术播报。",
].join("\n");
